...
const parser = csv.parse({ columns: true }, (err, data) => {
    ...
    for (const i in data) {
        if (data[i].iso) {
            ...
            if (
                checkEmpty(data[i].publisher) ||
                checkEmpty(data[i].website_link) ||
                checkBool(data[i].ocds_historic_data) ||
                checkBool(data[i].ocds_ongoing_data) ||
                checkBool(data[i].ocds_implementation)
            ) {
