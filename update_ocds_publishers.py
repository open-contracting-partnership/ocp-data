...
def update_ocds_publishers(filename):
    ...
    publishers = pd.read_csv(filename,
                             usecols=['Publisher or system name (see note if different from organization)',
                                      'Country (or "Multiple")',
                                      'Conforms', 'Status', non_mvp_reason_column, date_first_mvp_column, 'URL',
                                      'Standard'], keep_default_na=False
                             )
    # Include only retrievable OCDS publishers
    publishers = publishers[(publishers["Standard"] == "OCDS") & (publishers["Conforms"] == "Yes") &
            ...
            with Path(file_name).open() as country_file:
                ...
                for _index, publisher in publishers[publishers_countries == countries[country]["name"]].iterrows():
                    country_data["results"]["publishers"].append({
                        "publisher": publisher["Publisher or system name (see note if different from organization)"],
                        "publisher_link": publisher["URL"],
                        "ocds_historic_data": True,
                        "ocds_ongoing_data":
                            publisher[non_mvp_reason_column] != "Active: Updated in previous four calendar quarters",
                        "ocds_implementation": False,
                        "year_first_implemented": publisher[date_first_mvp_column][0:4] if publisher[
                            date_first_mvp_column] else None
                    })
